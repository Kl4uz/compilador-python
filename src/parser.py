...
Constrói a AST (Árvore de Sintaxe Abstrata) a partir dos tokens
"""
...
def p_stmt_var_decl(p):
    ...
    if len(p) == 4:
        p[0] = ('var_decl', p[2], None)  # Declaração sem inicialização
    else:
        p[0] = ('var_decl', p[2], p[4])  # Declaração com inicialização
...
def parse(source_code: str) -> Optional[Any]:
    """
    Analisa sintaticamente o código fonte
    
    Args:
        source_code: Código fonte a ser analisado
    
    Returns:
        AST em formato de tupla (será convertida para ASTNode posteriormente)
    """
    try:
